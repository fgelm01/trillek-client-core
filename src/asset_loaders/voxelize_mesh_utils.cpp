...
bool line_in_triangle(
    ...
    auto perform_test = [&triangle_transformed](
            const float_vector2d& point)->bool {
        for(std::size_t i = 0; i < 3; ++i) {
            const std::size_t j = (i + 1) % 3;
            float_vector2d edge = triangle_transformed[j] - 
                    triangle_transformed[i];
    ...
    if(std::min(line_a_z, line_b_z) < 0 && 
        ...
        const float_vector2d intersect_point = line_a_transformed + 
                ...
                line_a_transformed);
        return perform_test(intersect_point);
    } else {
        return false;
    }
}
...
bool triangle_in_cube(const float_triangle3d& triangle, 
    ...
    for(std::size_t i = 0; i < 8; ++i) {
        ...
        for(std::size_t shift = 0; shift < 3; ++shift) {
            ...
            if(line_in_triangle(corner, corner2, 
                    triangle)) {
                return true;
            }
        }
